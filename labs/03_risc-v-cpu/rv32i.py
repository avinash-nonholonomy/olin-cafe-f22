...
def line_to_bits(line, labels={}, address=0):
    ...
    if instruction in rtypes:
        if len(args) != 3:
            raise LineException("R-type instructions require 3 arguments.", )
        ...
        bits = funct7 + rs2 + rs1 + \
            funct3_codes[instruction] + rd + op_codes[instruction]
    if instruction in itypes:
        if len(args) != 3:
            raise LineException("I-type instructions require 3 arguments.", )
        rd, rs1, imm12 = args
        rd = register_to_bits(rd)
        rs1 = register_to_bits(rs1)
        imm12 = BitArray(int=int(imm12), length=12)
        bits = imm12 + rs1 + \
            funct3_codes[instruction] + rd + op_codes[instruction]
    ...
    if instruction in stypes:
        ...
        bits = imm12[0:7] + rs2 + rs1 + funct3_codes[instruction] + \
            imm12[7:] + op_codes[instruction]
    ...
    if not bits:
        raise LineException(f"Instruction {instruction} was not handled.", )
    if bits.length != 32:
        raise LineException(
...
rtype_funct3_mapping = {'001': 'sll', '010': 'slt',
                        '011': 'sltu', '100': 'xor', '110': 'or', '111': 'and'}
itype_funct3_mapping = {'000': 'addi', '001': 'slli', '010': 'slti',
                        '011': 'sltiu', '100': 'xori', '110': 'ori', '111': 'andi'}
ltype_funct3_mapping = {'000': 'lb', '001': 'lh',
                        '010': 'lw', '100': 'lbu', '101': 'lhu'}
stype_funct3_mapping = {'000': 'sb', '001': 'sh', '010': 'sw'}
btype_funct3_mapping = {'000': 'beq', '001': 'bne',
...
def bits_to_line(bits, labels=None):
    ...
    if op_code.bin == '0100011':  # s-type
        ...
        imm12 = bits[31-31:31-25+1] + bits[31-11:31-11+7]
    ...
    if op_code == op_codes['jal']:
        tmp = bits[0:20]
        imm20 = bits[0:1] + bits[10:20] + bits[9:10] + bits[1:9]
        address = imm20*2
        if labels is not None:
            if address not in labels:
                labels[address] = f"LABEL_{len(labels)}"
